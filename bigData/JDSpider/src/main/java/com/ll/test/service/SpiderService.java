...
public class SpiderService {
	...
	public void spiderData(String url, Map<String, String> params,Map<String,String>headerMap) {
		//String html = HttpClientUtils.getHtmlStrByDriver(url, null, params);
		String html=HttpClientUtils.sendGet(url, headerMap, params);
		...
		for (Element element : elements) {
			try {
				...
				if (StringUtils.isBlank(imgUrl)) {
					imgUrl =element.select("div[class=p-img]").select("a").select("img").attr("data-lazy-img");
				}
				if(StringUtils.isBlank(imgUrl)) {
					imgUrl =element.select("div[class=p-img]").select("a").select("img").attr("source-data-lazy-img");
				}
				...
				String merchantUrl = element.select("div[class=p-shop]").select("a").attr("href");// 商家地址
				//说明是京东自营
				if(StringUtils.isBlank(merchantName)&&StringUtils.isBlank(merchantUrl)){
					merchantUrl=merchantName="自营";
				}else{
					merchantUrl=HTTPS_PROTOCOL+merchantUrl;
				}
